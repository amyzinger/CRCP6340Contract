...
error AmyZingerNFTContract__MaxSupplyReached ();
error AmyZingerNFTContract__ValueNotEqualPrice();
...
ReentrancyGuard {
    ...
    uint96 _royaltyBasis
        ...
        ERC721("AmyZingerNFTContract", "AZ")
        Ownable(msg.sender)
    {
    ...
    receive() external payable {
        revert AmyZingerNFTContract__WrongAvenueForThisTransaction();
    }
    ...
    function mintTo(
        ...
        if (tokenId >= i_max_tokens) {
            revert AmyZingerNFTContract__MaxSupplyReached ();
        }
        // make sure there us the right amount of money
        if (msg.value != i_mint_price) {
            revert AmyZingerNFTContract__ValueNotEqualPrice();
        }
        _tokenIdCounter++;
