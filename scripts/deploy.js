...
async function main() {
  ...
  const AmyZingerNFTContractFactory = await ethers.getContractFactory(
    "AmyZingerNFTContract"
  );
  // deploy
  const AmyZingerNFTContract = await AmyZingerNFTContractFactory.deploy(
    args.mint_price,
    args.max_tokens,
    args.base_uri,
    args.royaltyArtist,
  ...
  await AmyZingerNFTContract.deploymentTransaction().wait(15);
  ...
  for (let i = 0; i < 3; i++) {
    const transactionResponse = await AmyZingerNFTContract.mintTo(ipfs[i], {
